...
public class SnsLoginController {
	...
	public ModelAndView kakaologin(HttpServletRequest request) {
		...
		try {
			...
			sb.append("&client_id=2f249e8e63c23abbda96d05efa33f0e7");
			sb.append("&redirect_uri=http://localhost:8080/kakaologin");
			sb.append("&code=" + code);
			sb.append("&client_secret=AYg6TEkTvZ4MnG0fziWs3hkzkfJisDDj");
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				// 사용자 정보를 다른 컨트롤러에서 받기 위해 session에 저장하자
				Gson gson = new Gson();
	...
	public ModelAndView naverLogin(HttpServletRequest request) {
		...
		try {
			...
			StringBuffer sb = new StringBuffer();
			sb.append("grant_type=authorization_code");
			sb.append("&client_id=VklLQp2NB9yJ6m59cB1w");
			sb.append("&client_secret=8QEeYnPD7V");
			sb.append("&code=" + code);
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				// 해당 정보를 Ajax 컨트롤러에서 사용하기 위해 session에 저장하자
	...
	public ModelAndView daumAddrOK(AddrVO addrvo, HttpSession session) {
		...
		// 로그인 성공한 상태에서 session 안에 있는 사용자 정보 중 m_id 또는 사용자 테이블의 pk를 호출한다.
