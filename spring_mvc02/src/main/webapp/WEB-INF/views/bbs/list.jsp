						...
						<ol class="paging">
						    ...
						    <c:choose>
						    	<c:when test="${paging.beginBlock <= paging.pagePerBlock}">
						    		<li class="disalbe">이전으로</li>
						    	</c:when>
						    	<c:otherwise>
						    		<li><a href="/bbs?cPage=${(paging.beginBlock - paging.pagePerBlock)+(paging.pagePerBlock - 1)}">이전으로</a></li>
						    	</c:otherwise>
						    </c:choose>
						    <!-- 블록안에 들어간 페이지번호들 -->
						    <c:forEach begin="${paging.beginBlock}" end="${paging.endBlock}" step="1" var="k">
						    	 <!-- 현재페이지(링크X) 와 현재 페이지가 아닌것(링크O) 을 구분하자. -->
