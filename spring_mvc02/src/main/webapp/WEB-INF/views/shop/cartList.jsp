		...
		<tbody>
			<c:choose>
				<c:when test="${empty cart_list}">
					<tr><td colspan="6"><h3>장바구니가 비어있습니다.</h3></td></tr>
				</c:when>
				<c:otherwise>
					<c:forEach var="k" items="${cart_list}">
						<tr>
							...
							<td>시중가 : <fmt:formatNumber value="${k.p_price}" pattern="#,##0"/>원<br>
								<font color="blue">( 할인가 : <fmt:formatNumber value="${k.p_saleprice}" pattern="#,##0"/>원 )</font>
							</td>
							<td>
							...
							<td><button onclick="cart_delete(${cart_idx})">삭제</button></td>
						</tr>
